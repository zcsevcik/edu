...
bool process(params_t* args)
{
...
#ifndef _WIN32
    if (!ipk_fd_block_mode(STDOUT_FILENO, false)) {
        return false;
    }
#endif
    ...
    if (!rdt_server_start(rdt, WINDOW_SIZE)) {
        goto return_false;
    }
...
#else
    int nfds = max(STDOUT_FILENO, udt) + 1;
#endif
    fd_set rfds, wfds;
    ...
    while (!_terminate) {
        ...
        if (nsndbuf == 0) {
            ...
            rdt_server_step(rdt, rcvbuf, nrcvbuf, sndbuf, &nsndbuf);
        }
        else {
            rdt_server_step(rdt, rcvbuf, nrcvbuf, NULL, NULL);
        }
        ...
        if (nbuf < _countof(buf)) {
            size_t nread = rdt_data_read(rdt, buf + nbuf, _countof(buf) - nbuf);
            nbuf += nread;
        }
        ...
        if (numfd == -1) {
            ...
            if (nbuf > 0) {
#endif
                size_t nsend = fwrite(buf, sizeof(char), nbuf, stdout);
                _ipk_verbose_log("Write %zu to stdout.", nsend);

                nbuf -= nsend;
                if (nsend != nbuf) {
                    if (ferror(stdout) && !ipk_sock_is_recoverable(errno)) {
                        ipk_perror(errno);
                        // send RST
                    }
                }
...
int main(int argc, char* argv[])
{
    ...
    if (!parseargs(argc, argv, &params)) {
        ...
        fprintf(stdout, "%s", "Synopsis: rdtserver -s source_port -d dest_port\n");
        return EXIT_FAILURE;
    }
    ...
    if (params.action == ACT_HELP) {
        fprintf(stdout, "%s", 
            ...
            "Autor: Radek Sevcik, xsevci44@stud.fit.vutbr.cz\n"
            "\n"
            "Synopsis: rdtserver -s source_port -d dest_port\n"
            "\n"
            "Parametry:\n"
            "  -h\t\t\tVypise napovedu.\n"
