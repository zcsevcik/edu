...
<!--
 * ftp.xslt:     Sprava hostingu (ITU 2012)
 *
...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <form><div class="ftp">
      <xsl:apply-templates select="//account[@login = $login]"/>
    </div></form>
  </xsl:template>

  <xsl:template match="//account">
        <div class="domena">cesta:</div>
        ...
        <div class="text">max users: <xsl:value-of select="@max_users" /></div>
  ...
  <xsl:template match="user">
    <li>
        <div class="f_tname">uživatel:</div>

        <xsl:element name="input">
          <xsl:attribute name="type">text</xsl:attribute>
          <xsl:attribute name="class">f_name</xsl:attribute>
          <xsl:attribute name="value">
            <xsl:value-of select="@name" />
          </xsl:attribute>
        </xsl:element>

        <input class="editftp" type="submit" value="" title="Ulozit" />
        <div class="f_tprava">práva:</div>

        <xsl:element name="input">
          <xsl:attribute name="type">text</xsl:attribute>
          <xsl:attribute name="class">f_prava</xsl:attribute>
          <xsl:attribute name="value">
        ...
        <input class="editftp" type="submit" value="" title="Ulozit" />
        <div class="f_tquota">quota:</div>

        <xsl:element name="input">
          <xsl:attribute name="type">text</xsl:attribute>
          <xsl:attribute name="class">f_quota</xsl:attribute>
          <xsl:attribute name="value">
