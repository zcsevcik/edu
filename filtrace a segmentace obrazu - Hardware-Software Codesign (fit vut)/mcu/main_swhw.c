...
/***************************************************************************
 ...
 Vstupy:
   ...
   n           pocet polozek histogramu
***************************************************************************/
void print_results(unsigned long frame, long threshold, long *hist, int n) {
...
int main(void)
{
   ...
   /**************************************************************************/
   while (1) {
       int fpga_hist_wr_done;
       fpga_hist_wr_done = fpga_read(FPGA_ADDR_HISTOGRAM_WR_DONE);
       ...
       if (fpga_hist_wr_done == 1) {
           ...
           unsigned long frame;
            ...
            for (i = 0; i < PIXELS; ++i)
                histogram[i] = fpga_read(FPGA_ADDR_HISTOGRAM_BASE + i);

            fpga_write(FPGA_ADDR_HISTOGRAM_WR_DONE, 0);

            new_threshold = otsu(histogram, PIXELS);
            fpga_write(FPGA_ADDR_THRESHOLD, new_threshold);
            fpga_write(FPGA_ADDR_THRESHOLD_WR_DONE, 1);

            // frame je aktualne vykreslovany snimek, cili histogram plati pro snimek-1
            frame = fpga_read(FPGA_ADDR_FRAME);
            if ((frame % 100) == 1)
                print_results(frame - 1, new_threshold, histogram, PIXELS);
       }
